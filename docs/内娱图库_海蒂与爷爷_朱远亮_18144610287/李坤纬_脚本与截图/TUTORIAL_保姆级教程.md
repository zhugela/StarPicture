...
打开你的文件夹 `D:\code\StarPicture\docs\test\李坤纬_脚本与截图\`，里面已经有：
...
│   └── space_functional.postman_collection.json
│
├── 性能测试/                            ← 你要做：跑 JMeter + 截图
│   └── space_analyze_20concurrent.jmx
│
├── 接口测试/                            ← 你要做：跑 1 个用例 + 截图
│   └── space_api.postman_collection.json
│
└── 安全测试/                            ← 你要做：填报告 + 写 .scan
    └── 空间模块-安全报告模板.docx
```
...
| 目录 | 文件 | 数量 |
|---|---|---|
| 功能测试/ | TC-SP-002_创建空间_普通版_xxx.png ~ TC-SU-005_删除空间成员_xxx.png | **10 张** |
| 性能测试/ | summary_report_xxx.png | **1 张** |
| 接口测试/ | API-001_创建空间_缺Content-Type.png | **1 张** |
| 安全测试/ | 空间模块-安全报告.pdf | **1 个** |
| 安全测试/ | 空间模块.scan | **1 个** |
...
2. 顶部菜单 **File → Import**
3. 弹窗里点 **Upload Files**
4. 选文件：`D:\code\StarPicture\docs\test\李坤纬_脚本与截图\功能测试\space_functional.postman_collection.json`
5. 看到 "1 file uploaded" → 点右下角 **Import**
6. 左侧栏应该出现 `StarPicture_空间模块_功能测试` 集合
7. 点开它，看到 10 个用例（5 个 空间创建 + 5 个 空间成员管理）
...
### 2.3 跑第 1 个用例：TC-SP-001_获取空间等级列表

**步骤**：

1. 左侧集合展开 → 找到 **TC-SP-001_获取空间等级列表** → 点开
2. 右侧中部会显示 `GET  http://localhost:8123/api/space/list/level`
3. 中间 Body 标签保持空（GET 没 body）
4. 右侧 **Send** 按钮（蓝色）→ 点一下
5. 期待右下角 Response 区域显示 Status: `200 OK`, Body: `{"code": 0, "data": ["普通版", "专业版", "旗舰版"], "message": "ok"}`
...
7. 另存为 PNG 到：`D:\code\StarPicture\docs\test\李坤纬_脚本与截图\功能测试\TC-SP-001_获取空间等级列表.png`
...
| 用例 | 期望结果 |
|---|---|
| TC-SP-002 创建空间_普通版 | code=0, spaceId 存在 |
...
4. 顶部菜单 **File → Open**
5. 选文件：`D:\code\StarPicture\docs\test\李坤纬_脚本与截图\性能测试\space_analyze_20concurrent.jmx`
...
2. 右侧面板：
   ...
   - `Port Number`：`8123`
   - `Method`：`POST`
   - `Path`：`/space/analyze/usage`
3. 下方 Body Data 应该有：{"spaceId": 1}
...
4. 看 Summary Report 的结果：
   - `# Samples`：应该是 20
   - `Average`：响应时间（毫秒），应 < 1000ms（1 秒）
   - `90% Line`：应 < 2000ms（2 秒）
   - `Error %`：应 < 5%
   - `Throughput`：吞吐量（请求/秒）
...
3. 粘贴到画图 → 另存为 PNG 到：`D:\code\StarPicture\docs\test\李坤纬_脚本与截图\性能测试\summary_report_空间分析20并发.png`

**验证**：图里应能看到 "20"（样本数）、"0.0%" 或 "< 5%"（错误率）、1000ms 左右。
...
2. 选 `D:\code\StarPicture\docs\test\李坤纬_脚本与截图\接口测试\space_api.postman_collection.json`
3. 左侧出现 `StarPicture_空间模块_接口测试`

### 4.2 跑接口用例：API-001_创建空间_缺Content-Type

**步骤**：

1. 左侧点开 `StarPicture_空间模块_接口测试`
2. 找到 **API-001_创建空间_缺Content-Type** → 点开
3. 右侧中部：
   - 选 **Headers** 标签 → 把 `Content-Type` 这一行删掉（点 ×）
   - 切到 **Body** 标签 → 选 **raw** → 选 **JSON**
   - 写：`{"spaceName": "test", "spaceLevel": 0}`
4. 点 **Send**
5. 期待右下角 Response：Status: `415` 或服务端能解析
...
1. 同 Day 2 功能测试的截图方法
2. 保存到：`D:\code\StarPicture\docs\test\李坤纬_脚本与截图\接口测试\API-001_创建空间_缺Content-Type.png`
...
5. 安装浏览器代理扩展：
   ...
   - 点 Add New Proxy：
     ...
     - Port: `8080`
   - 点 Save
6. 在浏览器开新标签访问 `http://localhost:8123/api/space/add`
7. 在 Body 写：`{"spaceName": "<script>alert(1)</script>", "spaceLevel": 0}`
8. 后端应该返回 `code=0（但渲染时被转义，不弹窗）`，证明 XSS 注入 被防住了
9. **保存 .scan 文件**：
   - 切到 BurpSuite → **Target** 标签
   - 左侧 Site map 应该出现 `localhost:8123`
   - 在 Site map 里点开 → 找到 `/api/space/add`
   - 右键 → **Save item**
   - 选格式 **`xxx.scan`**
   - 保存到：`D:\code\StarPicture\docs\test\李坤纬_脚本与截图\安全测试\空间模块.scan`
...
2. 输入：
   ...
   - 测试项：
     1. 空间名称 XSS 注入：spaceName 输入 `<script>alert(1)</script>`，预期渲染时被转义，预期 code=0（但渲染时被转义，不弹窗），实际通过
   ```
3. 另存为 `空间模块.scan.md`
